---
name: agent_browser
description: agent-browser CLI for headless Chrome via shell. Snapshot-and-ref workflow, click/fill/extract, screenshots, multi-tab, multi-session, network mocking. Install via pip; invoke directly from terminal.
---
...
## Installation (bare-metal Linux)

```bash
pip install agent-browser
agent-browser install    # downloads Chromium, sets up daemon
```

After install, invoke directly from the terminal. If you have an HTTP/HTTPS
proxy (e.g. Caido at `127.0.0.1:48080`), set `http_proxy`/`https_proxy` env
vars — agent-browser picks them up automatically. Do not pass `--proxy`;
localhost (CDP) traffic is excluded via `NO_PROXY=localhost,127.0.0.1`.
...
`agent-browser screenshot` writes a PNG to disk. The terminal command
alone does **not** put the image into your context — chain it with
whatever image-viewer tool your agent environment provides:

```bash
agent-browser screenshot
# Outputs path like: /home/user/.agent-browser/screenshots/page-2024-01-01.png
```

Default output directory is `~/.agent-browser/screenshots/`.
Prefer the no-arg form (the CLI prints the full path on stdout).
If you need a specific filename, keep it inside that directory.
Never write screenshots to `/tmp` — some tooling rejects paths
